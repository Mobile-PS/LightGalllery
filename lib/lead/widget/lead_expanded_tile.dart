...
import 'package:light_gallery/model/LeadListModel.dart';

import '../../model/TaskListModel.dart';
import '../../utils/constant/const_color.dart';
import 'LeadRowComponent.dart';



class LeadExpandedTile extends StatelessWidget {
  LeadExpandedTile({Key? key, required this.quotesResults, required this.date, required this.Index, required this.onpress1, required this.userid, required this.onLeadstatus})
      : super(key: key);

  final   List<Leads> quotesResults;
  final String date;
  final int Index;
  final int userid;
  final Function(int mainindex,int subindex) onpress1;
  final Function(int mainindex,int subindex, String leadstatus) onLeadstatus;
  ...
  Widget build(BuildContext context) {
   ...
   /* quotesResults.forEach((element) {
    ...
    for(int i= 0; i< quotesResults.length; i++){
      _statusChild.add(LeadRowComponent(title: quotesResults[i].fullName.toString(),byvalue: quotesResults[i].assignedBy.toString(),tovalue: quotesResults[i].assignedTo.toString(),status: quotesResults[i].priority.toString(),mainindex: Index,subindex: i,taskstatus: quotesResults[i].status.toString(),createdbyID: quotesResults[i].createdBy.toString(),loguserID: userid.toString(),assignID:quotesResults[i].assignedId.toString(),leadstatus: quotesResults[i].status.toString(),specifier: quotesResults[i].specifier.toString(),mobile: quotesResults[i].mobileNo.toString(), onpress: (int mainindex,int subindex){
        onpress1(mainindex,subindex);
      },onStatuschange: (int mainindex,int subindex,String leadstatus){
