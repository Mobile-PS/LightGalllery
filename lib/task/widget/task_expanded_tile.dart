...
class TaskExpandedTile extends StatelessWidget {
  TaskExpandedTile({Key? key, required this.quotesResults, required this.date, required this.Index, required this.onpress1, required this.userid})
      : super(key: key);

  final   List<Tasks> quotesResults;
  final String date;
  final int Index;
  final int userid;
  ...
  Widget build(BuildContext context) {
   ...
   /* quotesResults.forEach((element) {
    ...
    for(int i= 0; i< quotesResults.length; i++){
      _statusChild.add(CustomRowComponent(title: quotesResults[i].title.toString(),byvalue: quotesResults[i].assignedBy.toString(),tovalue: quotesResults[i].assignedTo.toString(),status: quotesResults[i].priority.toString(),mainindex: Index,subindex: i,taskstatus: quotesResults[i].status.toString(),createdbyID: quotesResults[i].createdBy.toString(),loguserID: userid.toString(),assignID:quotesResults[i].assignedid.toString(),repeattype:quotesResults[i].repeat.toString() , onpress: (int mainindex,int subindex){
